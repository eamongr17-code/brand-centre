      ...
      {brandId && (
        <div>
          ...
          <div className="max-h-48 overflow-y-auto space-y-1 [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-thumb]:bg-[#2a2a2a] [&::-webkit-scrollbar-thumb]:rounded-full">
            {categories.length === 0 ? (
              <p className="text-xs text-[#505050] py-4 text-center">No asset categories found for this brand</p>
            ) : categories.map((cat) => (
              <button
                key={cat.id}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-[#505050]">{cat.description || "No description"}</p>
                </div>
              </button>
            ))}
          </div>

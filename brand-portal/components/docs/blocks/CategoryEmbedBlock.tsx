    ...
    <div className="rounded-xl border border-white/[0.07] bg-white/[0.02] overflow-hidden">
      ...
      <div className="flex items-center justify-between px-5 py-3 border-b border-white/[0.05]">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-[#f0f0f0]">{category.name}</p>
            <p className="text-[10px] text-[#505050]">{visibleAssets.length} assets · {brand.name}</p>
          </div>
        </div>
        <Link
          href={portalPath(`/${brand.slug}/${category.slug}`)}
          className="text-xs text-[#555] hover:text-[#f0f0f0] transition-colors font-semibold"
        >
          View all
        </Link>
      </div>
      ...
      {visibleAssets.length === 0 ? (
        <div className="px-5 py-8 text-center text-sm text-[#505050]">No assets in this category yet</div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-px bg-white/[0.04]">
          {visibleAssets.slice(0, 6).map((asset) => (
            <div key={asset.id} className="bg-[#111] p-3 group/asset">
              <div className="aspect-square rounded-lg overflow-hidden bg-white/[0.02] border border-white/[0.05] mb-2">
                <FadeImg
              ...
              <div className="flex items-center justify-between mt-1">
                <span className="text-[10px] text-[#505050]">{asset.fileType}</span>
                <a href={asset.downloadUrl || "#"} className="text-[#505050] hover:text-[#f0f0f0] transition-colors" title="Download">
                  <Download size={11} />
                </a>
              </div>
            </div>
      ...
      {visibleAssets.length > 6 && (
        <div className="px-5 py-2.5 border-t border-white/[0.05] text-center">
          <Link
            href={portalPath(`/${brand.slug}/${category.slug}`)}
            className="text-xs text-[#555] hover:text-[#f0f0f0] transition-colors font-semibold"
          >

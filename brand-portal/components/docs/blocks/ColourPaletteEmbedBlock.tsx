...
export default function ColourPaletteEmbedBlock({ block }: { block: ColourPaletteEmbedBlockType }) {
  ...
  return (
    <div className="rounded-xl border border-white/[0.07] bg-white/[0.02] overflow-hidden">
      ...
      <div className="flex items-center justify-between px-5 py-3 border-b border-white/[0.05]">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-[#505050]">{colours.length} colours · {brand.name}</p>
          </div>
        </div>
        <Link
      ...
      {/* Colour swatches */}
      {colours.length === 0 ? (
        <div className="px-5 py-8 text-center text-sm text-[#505050]">No colours in this palette yet</div>
      ) : (
        <div className="p-4 grid grid-cols-2 sm:grid-cols-3 gap-3">
          {colours.map((colour) => (
            <button
              ...
              className="group flex items-center gap-3 bg-white/[0.02] border border-white/[0.05] rounded-lg px-3 py-2.5 hover:border-white/[0.1] transition-all duration-200 text-left"
            >
              <div
